...
start_docker_image() {
  ...
  ssh -o StrictHostKeyChecking=no -i key.pem ubuntu@"$1" "docker run -e TELEGRAM_BOT_TOKEN=$TELEGRAM_BOT_TOKEN -e DB_CONNECTION_URL=$DB_CONNECTION_URL -d weaxme/funny-bot:$VERSION"
}
